...
export const createTaskSchema = (
  ...
  let schema = z.object({
    ...
    title: enforceRequiredFields ? z.string().min(3, { message: 'Title must be at least 3 characters.' }) : z.string().optional().catch(''),
    description: enforceRequiredFields ? z.string().min(3, { message: 'Description must be at least 3 characters.' }) : z.string().optional().catch(''),
    status: enforceRequiredFields ? z.string().min(1, 'Status is required.') : z.string().optional().catch(''),
      ...
      if (fieldName in schema.shape) {
        ...
        if (fieldSchema) {
          if (field.key === 'deploymentStatus') {
            fieldSchema = fieldSchema.refine((val: Record<string, boolean | undefined> | undefined) => val !== undefined && Object.values(val).some((v) => v === true), {
            ...
            fieldSchema = fieldSchema.refine((val: unknown) => val !== undefined && val !== null && val !== '', {
              message: `${field.label} is required.`,
            });
          }
