...
export default function TaskPage() {
  ...
  const renderCustomFieldValue = (task: Task, key: string, value: any) => {
      ...
      switch (fieldConfig.type) {
          case 'text':
              const aliasKey = `${key}_alias`;
              const alias = task.customFields?.[aliasKey];
              if (fieldConfig.baseUrl && value) {
                  const url = `${fieldConfig.baseUrl}${value}`;
                  return <a href={url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-2"><ExternalLink className="h-4 w-4"/> {alias || value}</a>
              }
              return String(value);
          case 'date':
          ...
          case 'checkbox':
              return value ? 'Yes' : 'No';
          case 'url':
              const urlAliasKey = `${key}_alias`;
              const urlAlias = task.customFields?.[urlAliasKey];
              return <a href={value} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{urlAlias || value}</a>
          case 'multiselect':
          case 'tags':
                                ...
                                <CardContent className="space-y-4">
                                    {groupedCustomFields[groupName].map(field => (
                                        <div key={field.key}>
                                            ...
                                            <div className="text-sm text-foreground min-w-0 break-words">{renderCustomFieldValue(task, field.key, task.customFields?.[field.key])}</div>
                                        </div>
                                    ))}
                                </CardContent>
                            </Card>
