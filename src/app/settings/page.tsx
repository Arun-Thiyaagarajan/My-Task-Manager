                        ...
                        <CardContent className="space-y-6">
                            <div className="space-y-3">
                                ...
                                <RadioGroup value={theme} onValueChange={setTheme} className="grid grid-cols-3 gap-2">
                                    <button onClick={() => setTheme('light')} className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all gap-2", theme === 'light' ? "bg-primary/5 border-primary shadow-sm" : "bg-muted/30 border-transparent")}><Sun className={cn("h-5 w-5", theme === 'light' ? "text-primary" : "text-muted-foreground")} /><span className={cn("text-[10px] font-bold uppercase", theme === 'light' ? "text-primary" : "text-muted-foreground")}>Light</span></button>
                                    <button onClick={() => setTheme('dark')} className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all gap-2", theme === 'dark' ? "bg-primary/5 border-primary shadow-sm" : "bg-muted/30 border-transparent")}><Moon className={cn("h-5 w-5", theme === 'dark' ? "text-primary" : "text-muted-foreground")} /><span className={cn("text-[10px] font-bold uppercase", theme === 'dark' ? "text-primary" : "text-muted-foreground")}>Dark</span></button>
                                    <button onClick={() => setTheme('system')} className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all gap-2", theme === 'system' ? "bg-primary/5 border-primary shadow-sm" : "bg-muted/30 border-transparent")}><Monitor className={cn("h-5 w-5", theme === 'system' ? "text-primary" : "text-muted-foreground")} /><span className={cn("text-[10px] font-bold uppercase", theme === 'system' ? "text-primary" : "text-muted-foreground")}>System</span></button>
                                </RadioGroup>
                            </div>
                            ...
                            <div className="space-y-2"><Label className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Workspace Icon</Label><div className="flex gap-2"><Input value={isDataURIIcon ? '' : (appIcon || '')} onChange={e => setAppIcon(e.target.value)} placeholder={isDataURIIcon ? "Custom image uploaded" : "Emoji or URL..."} className="h-10 flex-1 font-normal" /><Button variant="outline" size="icon" className="h-10 w-10 shrink-0" onClick={() => iconFileInputRef.current?.click()}><Upload className="h-4 w-4" /></Button><input type="file" ref={iconFileInputRef} onChange={handleIconUpload} className="hidden" accept="image/*" /></div></div>
                ...
                {activeMobileSection === 'environments' && (
                    <Card className="border shadow-lg rounded-3xl">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex gap-2"><Input placeholder="New environment..." className="h-10 text-xs rounded-xl" value={newEnvName} onChange={e => setNewEnvName(e.target.value)} /><Button size="sm" className="h-10 px-4 shrink-0 rounded-xl font-bold" onClick={handleAddEnv}>Add</Button></div>
                        </CardContent>
                    </Card>
                )}
                ...
                <CardHeader className="pb-6">
                    ...
                    <div className="pt-6">
                        <div className="relative">
                            ...
                            <Input placeholder="Search fields..." className="pl-10 h-12 text-base font-normal" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                        </div>
                    </div>
                </CardHeader>
